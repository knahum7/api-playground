...
const Page = () => {
  ...
  const [editWorkingHours, setEditWorkingHours] = useState<WorkingHour[] | null>(null);
  ...
  const handlePut = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/getir/restaurants/working-hours", {
        ...
        body: JSON.stringify({ restaurantWorkingHours: editWorkingHours }),
      });
      const data = await res.json();
  ...
  // Edit handler
  const handleEditChange = (day: number, field: string, value: string | boolean) => {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-gray-100">
      ...
      <div className="w-full max-w-3xl p-8 bg-white rounded shadow-md flex flex-col gap-8">
        {/* GET Form */}
        <form className="flex flex-col gap-4" onSubmit={handleGet} aria-label="Getir Working Hours GET form">
          ...
          {(getError || getResponse) && (
            <div className={`mt-2 p-3 rounded text-sm ${getError ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`} aria-live="polite">
              {getError ? (
                <span>{getError}</span>
              ) : (
                <div>
                  <div className="mb-2 font-medium">Restaurant Working Hours:</div>
                  <ul className="list-disc pl-5 mb-2">
                    {getResponse?.restaurantWorkingHours?.map((h) => (
                      <li key={h.day}>
                        <span className="font-semibold">{DAY_LABELS[h.day]}:</span> {h.workingHours.closed ? "Closed" : `${h.workingHours.startTime} - ${h.workingHours.endTime}`}
                      </li>
                    ))}
                  </ul>
                  <div className="mb-2 font-medium">Restaurant Courier Hours:</div>
                  <ul className="list-disc pl-5">
                    {getResponse?.restaurantCourierHours?.map((h) => (
                      <li key={h.day}>
                        <span className="font-semibold">{DAY_LABELS[h.day]}:</span> {h.workingHours.closed ? "Closed" : `${h.workingHours.startTime} - ${h.workingHours.endTime}`}
                      </li>
                    ))}
        ...
        {/* PUT Form */}
        {editWorkingHours && (
          <form className="flex flex-col gap-4 border-t pt-8" onSubmit={handlePut} aria-label="Getir Working Hours PUT form">
            <h2 className="text-xl font-bold text-pink-700">Update Working Hours</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {editWorkingHours.map((h) => (
                <div key={h.day} className="border rounded p-4 flex flex-col gap-2 bg-gray-50">
                  <div className="font-semibold mb-1">{DAY_LABELS[h.day]}</div>
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={h.workingHours.closed}
                      onChange={e => handleEditChange(h.day, "closed", e.target.checked)}
                      className="form-checkbox h-4 w-4 text-pink-600"
                      aria-label="Closed"
                    />
                    <span>Closed</span>
                  </label>
                  {!h.workingHours.closed && (
                    <>
                      <label className="flex flex-col">
                        ...
                        <input
                          ...
                          onChange={e => handleEditChange(h.day, "startTime", e.target.value)}
                      ...
                      <label className="flex flex-col">
                        ...
                        <input
                          ...
                          onChange={e => handleEditChange(h.day, "endTime", e.target.value)}
                          className="border rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-pink-500"
                          aria-label="End Time"
                        />
                      </label>
            ...
            {(putError || putResponse) && (
              <div className={`mt-2 p-3 rounded text-sm ${putError ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`} aria-live="polite">
                {putError ? <span>{putError}</span> : <span>{putResponse}</span>}
              </div>
            )}
          </form>
        )}
